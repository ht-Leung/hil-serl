...
def load_resnet18_params(agent, image_keys=("image",), public=True):
    """
    Load pretrained resnet18 params from github release to an agent.
    :return: agent with pretrained resnet18 params
    """
    file_name = "resnet18_params.pkl"
    if not public:  # if github repo is not public, load from local file
        with open(file_name, "rb") as f:
            encoder_params = pkl.load(f)
    else:  # when repo is released, download from url
        ...
        # Check if the file exists
        if os.path.exists(file_path):
            print(f"The ResNet-18 weights already exist at '{file_path}'.")
        else:
            url = f"https://github.com/rail-berkeley/serl/releases/download/resnet18/{file_name}"
            print(f"Downloading file from {url}")
    ...
    param_count = sum(x.size for x in jax.tree.leaves(encoder_params))
    print(
        f"Loaded {param_count/1e6}M parameters from ResNet-18 pretrained on ImageNet-1K"
    )
