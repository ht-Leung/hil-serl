#!/usr/bin/env python3
"""
Dynamic ROI wrapper for HIROLUnifiedEnv
...
from experiments.hirol_unifined.wrapper import HIROLUnifiedEnv


class HIROLUnifiedEnvWithDynamicROI(HIROLUnifiedEnv):
    """
    Extended HIROLUnifiedEnv with dynamic ROI support
    ...
    def __init__(self, **kwargs):
        ...
        print("✓ Dynamic ROI initialized for HIROLUnifiedEnv")
...
def create_dynamic_roi_env(**kwargs):
    ...
    """
    return HIROLUnifiedEnvWithDynamicROI(**kwargs)
...
def get_environment_with_dynamic_roi(fake_env=False, save_video=False, classifier=True):
    ...
    """
    from experiments.hirol_unifined.config import TrainConfig
    
    config = TrainConfig()
    
    # Create environment with dynamic ROI
    env = HIROLUnifiedEnvWithDynamicROI(
