...
class DefaultEnvConfig:
    ...
    # Camera configuration - update these serial numbers to match your setup
...
class FR3Env(gym.Env):
    ...
    def __init__(
        ...
        # Ideal pose tracking (avoids accumulating execution errors)
    ...
    def get_im(self) -> Dict[str, np.ndarray]:
        ...
        if self.robot is None:
            # Return dummy images for fake env
            return {key: np.zeros(self.observation_space["images"][key].shape, dtype=np.uint8) 
                    for key in self.config.REALSENSE_CAMERAS}
    ...
    def _send_pos_command(self, pos: np.ndarray) -> None:
        ...
        if self.robot is not None:
            # Don't call _recover() on every command - it causes excessive reinitialization
            # Only call _recover() when there's an actual error
            # self._recover()  # DISABLED - only call when actually needed
