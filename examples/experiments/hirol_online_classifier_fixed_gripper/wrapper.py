...
FilePath: /code/hil-serl/examples/experiments/hirol_fixed_gripper/wrapper.py
'''
"""HIROL Fixed Gripper Environment Wrapper"""
...
class HIROLFixedGripperEnv(HIROLEnv):
    """
    HIROL Fixed Gripper task environment.
    
    This environment is designed for manipulation tasks with a fixed gripper state,
    where the gripper remains closed throughout the task. Suitable for tasks like
    pushing, nudging, or manipulation without grasping.
    """
...
class GripperInitWrapper(gym.Wrapper):
    """
    Simple wrapper to control gripper initial state without changing action space.
    Works with both 6-dim and 7-dim action spaces.
    Uses the _pre_enter_hook to set gripper state BEFORE waiting for Enter.
    """
    def __init__(self, env, init_mode="close"):
        """
        Args:
            env: Base environment
            init_mode: "open", "close", or "none"
        """
        super().__init__(env)
        self.init_mode = init_mode
        
    def _handle_gripper_init(self):
        """Hook function called by base environment before waiting for Enter"""
        robot = getattr(self.env.unwrapped, 'robot', None)
        if robot is not None and self.init_mode != "none":
            if self.init_mode == "close":
                print("[GripperInitWrapper] Closing gripper before episode start")
                robot.close_gripper()
            elif self.init_mode == "open":
                print("[GripperInitWrapper] Opening gripper before episode start")
                robot.open_gripper()
        
    def reset(self, **kwargs):
        # Set up the hook to be called before waiting for Enter
        self.env.unwrapped._pre_enter_hook = self._handle_gripper_init
